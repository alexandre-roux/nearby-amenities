...
export const CACHE = new Map(); // key -> { at: number, data: any }
...
export function keyFor(lat, lon, radius, filters) {
    const f = normalizeFilters(filters);
    return `${roundCoord(lat)},${roundCoord(lon)}|${roundRadius(radius)}|t${+f.toilets}f${+f.fountains}g${+f.glass}`;
}

export function buildOverpassQL(lat, lon, radiusMeters = 1000, opts = {}) {
...
export async function fetchOverpass(lat, lon, radius = 1000, signal, opts) {
